...
class Openpose(object):
        ...
        for subset in subsets:
            ...
            for joint_index in subset[:18].astype('i'):
                if joint_index >= 0:
                    ...
                    # successful limbs count
                    joint.append(2)
                    # scores
                    joint.append(all_peaks[joint_index][3])
                    joints.append(joint)
                else:
                    joints.append([0, 0, 0, 0])
            person_pose_array.append(np.array(joints))
        person_pose_array = np.array(person_pose_array)
        person_pose_array_with_scores = np.delete(person_pose_array, 2, axis=2)
        return person_pose_array, person_pose_array_with_scores
    ...
    def detect(self, orig_img, precise=False):
        ...
        poses, poses_with_scores = self.subsets_to_pose_array(subsets, all_peaks)
        scores = subsets[:, -2]
        return poses, scores, poses_with_scores
...
def draw_person_pose(orig_img, poses):
    ...
    # joints
    for pose in poses.round().astype('i'):
        for i, ((x, y, v, score), color) in enumerate(zip(pose, joint_colors)):
            if v != 0:
